...
export const ExerciseItem = memo((props: ExerciseItemProps) => {
  ...
  return (
    <Item
      onClick={onClick}
      className={cn(
        // Добавлен класс group для ховер-эффектов дочерних элементов и стили референса
        ...
        itemType === "large" && "p-3 h-25 sm:h-28 w-full", // Твои размеры и структура без изменений
      )}
      variant={"outline"}
    >
      {/* Контейнер для картинки/скелетона */}
      <div className="h-full aspect-square flex items-center justify-center">
        {/* Добавлен правильный бордер для скелетона, сочетающийся с референсом */}
        <Skeleton className="h-full w-full rounded-md border border-slate-100/50 dark:border-slate-800/40" />
      </div>

      <ItemContent className="min-w-0">
        {/* Название подсвечивается цветом indigo при наведении на карточку */}
        ...
        {muscle_group_name && (
          /* Описание приведено к приглушенному slate-тону референса */
